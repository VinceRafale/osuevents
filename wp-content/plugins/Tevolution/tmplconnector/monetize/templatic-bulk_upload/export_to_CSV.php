...
$authorsql_conditions = " where p.post_type = '".@$_REQUEST['post_type_export']."' and p.post_status='publish' and p.ID = pm.post_id";
  	...
  	if($authorinfo)
	{
		...
		/*	get all custom fields of post START	*/
			...
			{				
				...
				{
					$post_value=get_post_meta($postObj->ID,$key,true);		
		...
		/*	get all custom fields of post END	*/		
			...
			if($templatic_counter == 1){				
				$header_top =  "templatic_post_author,templatic_post_date,templatic_post_title,templatic_post_category,templatic_img,templatic_post_tags,templatic_post_content,templatic_post_excerpt,templatic_post_status,templatic_comment_status,templatic_ping_status,templatic_post_name,templatic_post_modified,templatic_post_guid,templatic_post_type,templatic_post_comment_count,templatic_comments_data";
			...
			$content_1 =  array("$postObj->post_author","$post_date","$post_title","$category","$image","$tags","$postObj->post_content","$post_excerpt","$postObj->post_status","$postObj->comment_status","$postObj->ping_status","$postObj->post_name","$postObj->post_modified","$postObj->guid","$postObj->post_type","$postObj->comment_count");
