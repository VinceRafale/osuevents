...
function templatic_twitter_messages($options) {
	...
	$out = '<ul class="templatic_twitter_widget">';
...
function time_elapsed_string($ptime) {
    ...
    if ($etime < 1) {
        return __('0 seconds',DOMAIN);
    }
    
    $a = array( 12 * 30 * 24 * 60 * 60  =>  __('year',DOMAIN),
                30 * 24 * 60 * 60       =>  __('month',DOMAIN),
                24 * 60 * 60            =>  __('day',DOMAIN),
                60 * 60                 =>  __('hour',DOMAIN),
                60                      =>  __('minute',DOMAIN),
                1                       =>  __('second',DOMAIN)
                );
    ...
    foreach ($a as $secs => $str) {
        ...
        if ($d >= 1) {
            $r = round($d);
            return __("$r $str".($r > 1 ? 's' : ''),DOMAIN).' '.__("ago",DOMAIN);
		}
    }
}
//Function to convert date to time ago format
...
class templatic_twiter extends WP_Widget {
    ...
    function templatic_twiter() {
		$this->options = array(
			...
			array(
				'name'	=> 'num',
				'label'	=> __( 'Show # of Tweets', DOMAIN ),
				'type'	=> 'text'
			),
			array(
				'name'	=> 'linked',
				'label'	=> __( 'Show this linked text for each Tweet', DOMAIN ),
				'type'	=> 'text'
			),
			array(
				'name'	=> 'follow_text',
			...
			array(
				'name'	=> 'link_user',
				'label'	=> __( 'Link below tweets with Twitter user', DOMAIN ),
				'type'	=> 'checkbox'
			),			
			
			array(
				'name'	=> 'hyperlinks',
				'label'	=> __( 'Find and show hyperlinks', DOMAIN ),
				'type'	=> 'checkbox'
			),			
        ...
        $widget_ops = array('classname' => 'widget Templatic twitter', 'description' => __('Show your latest twitter tweets on your site. .') );
		$this->WP_Widget('templatic_twiter', __('T &rarr; Latest Twitter Feed'), $widget_ops);
    }
    ...
    function widget($args, $instance) {		
		...
		_e(templatic_twitter_messages($instance),DOMAIN);
		if($instance['follow_text'] !="" ){
            
			echo '<a href="http://twitter.com/' . $instance['username'] . '" class="twitter_title_link follow_us_twitter" '.$link_target.'>'.  $instance['follow_text'] . '</a>';
    ...
    function form($instance) {
		if (empty($instance)) {
			$instance['title']			= __( 'Last Tweets', DOMAIN );			
			$instance['username']		= '';
			$instance['num']			= '5';			
			$instance['linked']			= '#';
