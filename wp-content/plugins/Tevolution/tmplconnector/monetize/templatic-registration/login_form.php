...
case 'retrievepassword' :
	if ( $http_post ) {
		...
		if ( !is_wp_error($errors) ) {
			global $General;
			wp_redirect($General->get_url_login(home_url()).'/?ptype=login&action=login&checkemail=confirm');
			exit();
		}
	}
	...
	$message = '<div class="success_msg">'.__('Please enter your username or e-mail address. You will receive a new password via e-mail.',DOMAIN).'</div>';
...
case 'rp' :
	...
	global $General;
	if ( ! is_wp_error($errors) ) {
		wp_redirect($General->get_url_login(home_url()).'/?ptype=login&action=login&checkemail=newpass');
		exit();
	}

	wp_redirect($General->get_url_login(home_url()).'/?ptype=login&action=lostpassword&error=invalidkey');
	exit();
...
case 'register' :
	if ( !get_option('users_can_register') ) {
		wp_redirect(home_url().'/?ptype=login&registration=disabled');
		exit();
	}
	...
	if ( $http_post &&  @$_POST) {
		...
		}else
		{
			if ( !is_wp_error($errors) ) {
				global $General;
				wp_redirect($General->get_url_login(home_url()).'/?ptype=login&action=login&checkemail=registered');
				exit();
			}	
		}
	...
	//login_header(__('Registration Form',DOMAIN), '<p class="message register">' . __('Register For This Site',DOMAIN) . '</p>', $errors);
break;
...
<div class="login_form_box">
  <form name="loginform" id="loginform" action="<?php echo get_settings('home').'/index.php?ptype=login&amp;ptype1='.$_REQUEST['ptype'].$lang; ?>" method="post" >
    <div class="form_row clearfix">
      <label><?php _e(USERNAME_TEXT,DOMAIN); ?> <span class="indicates">*</span> </label>
    ...
    <div class="form_row clearfix">
      <label> <?php _e(PASSWORD_TEXT,DOMAIN); ?> <span class="indicates">*</span> </label>
    ...
    <div class="form_row rember clearfix">
      <label>
      	...
      	<?php _e(REMEMBER_ON_COMPUTER_TEXT,DOMAIN); ?> 
      </label>	
      <a href="javascript:void(0);showhide_forgetpw();" class="forgot_password" ><?php _e(FORGOT_PW_TEXT,DOMAIN);?></a>
    </div>
    <!-- <a  href="javascript:void(0);" onclick="chk_form_login();" class="highlight_button fl login" >Sign In</a>-->
	
	<div class="form_row ">
    <input class="b_signin_n" type="submit" value="<?php _e(SIGN_IN_BUTTON,DOMAIN);?>"  name="submit" />
  ...
  <div id="lostpassword_form" <?php if($display_style != '') { echo $display_style; } else { echo 'style="display:none;"';} ?> >
    <h2><?php  _e(FORGOT_PW_TEXT,DOMAIN);?></h2>
    <form name="lostpasswordform" id="lostpasswordform" action="<?php echo home_url().'/?ptype=login&amp;action=lostpassword'; ?>" method="post">
      <div class="form_row clearfix">
        <label> <?php  _e(USERNAME_EMAIL_TEXT,DOMAIN); ?>: </label>
        <input type="text" name="user_login" id="user_login1" value="<?php if(isset($user_login))echo esc_attr($user_login); ?>" size="20" class="textfield" />
