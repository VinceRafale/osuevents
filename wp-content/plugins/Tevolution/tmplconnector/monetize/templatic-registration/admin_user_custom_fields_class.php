...
class wp_list_custom_user_field extends WP_List_Table 
{
	...
	* Call a function that will return all the data in an array and we will assign that result to a variable $custom_user_field_data. FIRST OF ALL WE WILL FETCH DATA FROM POST META TABLE STORE THEM IN AN ARRAY $custom_user_field_data */
	function fetch_custom_user_fields_meta_data( $post_id = '' ,$post_title = '' , $post_status = '')
	{
		...
		$meta_data = array(
			'ID'				=> $post_id,
			'title'				=> '<strong><a href="'.site_url().	'/wp-admin/admin.php?page=user_custom_fields&action=addnew&cf='.$post_id.'">'.$post_title.'</a></strong>',
			'type' 				=> $ctype,
			'active' 			=> $post_status,
	...
	function custom_user_fields()
	{
		...
		while ($post_query->have_posts()) : $post_query->the_post();
				$custom_user_field_data[] = $this->fetch_custom_user_fields_meta_data($post->ID,$post->post_title,$post->post_status);
		endwhile;
