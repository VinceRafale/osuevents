...
function retrieve_password() {
	...
	$message  = __('<p><b>Your login Information :</b></p>',DOMAIN);
	$message  .= '<p>'.sprintf(__('Username: ',DOMAIN).'%s', $user->user_login) . "</p>";
	$message .= '<p>'.sprintf(__('Password: ',DOMAIN).'%s', $new_pass) . "</p>";
	$message .= __('<p>You can login to : <a href="'.site_url().'/?ptype=login">Login</a> or the URL is :  '.site_url().'/?ptype=login</p>',DOMAIN);
	$message .= __('<p>Thank You,<br> '.get_option('blogname').'</p>',DOMAIN);
	$user_email = $user_data->user_email;
...
function register_new_user($user_login, $user_email) {
	...
	if( $tmpdata['recaptcha'] == 'recaptcha')
	{
		...
		if(file_exists(ABSPATH.'wp-content/plugins/wp-recaptcha/recaptchalib.php') && is_plugin_active('wp-recaptcha/wp-recaptcha.php') && in_array('registration',$display))
		{
			...
			if (!$resp->is_valid ) {
				wp_redirect(home_url().'/?ptype=login&ecptcha=captcha');
				exit;
			}
		}
	}
	elseif($tmpdata['recaptcha'] == 'playthru')
	{
		...
		if(file_exists(ABSPATH.'wp-content/plugins/are-you-a-human/areyouahuman.php') && is_plugin_active('are-you-a-human/areyouahuman.php')  && in_array('registration',$display))
		{
			...
			if($score == '')
			{
				wp_redirect(home_url().'/?ptype=login&ecptcha=play');
				exit;	
			}
		}
	}
	...
	update_usermeta($user_id,'user password',$user_pass);
	...
	if ( $user_id ) {
		...
		if($subject=="" && $client_message=="")
		{
			...
			<p>[#site_name#]</p>',DOMAIN);
			$filecontent_arr1 = explode('[SUBJECT-STR]',$client_message);
			$filecontent_arr2 = explode('[SUBJECT-END]',$filecontent_arr1[1]);
			$subject = $filecontent_arr2[0];
			if($subject == '')
			{
				$subject = __("Registration Email",DOMAIN);
			}
			
			$client_message = $filecontent_arr2[1];
		}
		$store_login_link = '<a href="'.home_url().'/?ptype=login&akey='.$activation_key.'&uid='.base64_encode($user_id).'">'.home_url().'/?ptype=login&akey='.$activation_key.'&uid='.base64_encode($user_id).'</a>';
		$store_login = sprintf(__('<a href="'.home_url().'/?ptype=login&akey='.$activation_key.'&uid='.base64_encode($user_id).'">'.'Click Login'.'</a>',DOMAIN));
...
function init_templ_reg()
{
	global $page_title, $wpdb, $wp_rewrite;	
	$page_title = SIGN_IN_PAGE_TITLE;
	...
	switch ($action) {	
		...
		case 'retrievepassword' :
				if ( $http_post ) {
					...
					if ( !is_wp_error($errors) ) {
						wp_redirect(home_url().'/?ptype=login&page1=sign_in&checkemail=confirm');
						exit();
					}else
					{
						wp_redirect(home_url().'/?ptype=login&page1=sign_in&emsg=fw');
						exit();
					}
				}
				if ( isset($_GET['error']) && 'invalidkey' == $_GET['error'] ) $errors->add('invalidkey', __('Sorry, that key does not appear to be valid.',DOMAIN));
		...
		case 'rp' :
				...
				if ( ! is_wp_error($errors) ) {
					wp_redirect(home_url().'/?ptype=login&action=login&checkemail=newpass');
					exit();
				}			
				wp_redirect(home_url().'/?ptype=login&action=lostpassword&page1=sign_in&error=invalidkey');
				exit();		
			break;		
		case 'register' :			
				$user_login = '';
				...
				if ( !get_option('users_can_register') ) {
					wp_redirect(home_url().'?ptype=login&page1=sign_up&emsg=regnewusr');
					exit();
				}
				if ( $http_post ) {
		...
		default:
			...
			if ( isset( $_REQUEST['redirect_to'] ) && $_REQUEST['redirect_to'] != "") {
				...
				if($_REQUEST['ptype1'] != '') {
					...
					$redirect_to = 	Unaccent(get_author_posts_url($user->data->ID));
				}
				...
				if(isset($user->ID))
					$_REQUEST['redirect_to']=Unaccent(get_author_posts_url($user->ID));
			}
			...
			if(is_wp_error($user))
			{
				if(isset($_SERVER['HTTP_REFERER']) && strstr($_SERVER['HTTP_REFERER'],'ptype=submition') && $_POST['log']!='' && $_POST['pwd']!='')
				{
			...
			if((isset($_POST['log']) && $errors) || (isset($_POST['log'])=='' && isset($_REQUEST['testcookie'])))
			{	
				...
				{ 
					wp_redirect(home_url().'?ptype=login&page1=sign_in&emsg=1');
				}
				exit;
			}
		break;
		  ...
		  <div class="post-meta">
			  <?php if ( get_option('templatic-breadcrumbs' )) {  ?>
				<div class="breadcrumb clearfix">
					<div class="breadcrumb_in"><a href="<?php echo site_url(); ?>"><?php _e('Home',DOMAIN); ?></a> &raquo; <?php _e(SIGN_IN_PAGE_TITLE,DOMAIN); ?> </div>
				</div>
			<?php } ?>
			<?php //templ_page_title_above(); //page title above action hook ?>
			<?php //add_filter('wp_title',SIGN_IN_PAGE_TITLE); //page tilte filter ?>
...
add_action('init', 'init_templ_reg',11); 
?>
