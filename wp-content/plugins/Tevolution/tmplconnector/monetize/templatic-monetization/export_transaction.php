...
if($transinfo)
{
	foreach($transinfo as $priceinfoObj)
	{
		...
		echo "$post_title,".date('d/m/Y',strtotime($priceinfoObj->payment_date)).",$priceinfoObj->billing_name,$priceinfoObj->payment_method,".tmpl_fetch_currency().number_format($priceinfoObj->payable_amt,2)." \r";
 }
