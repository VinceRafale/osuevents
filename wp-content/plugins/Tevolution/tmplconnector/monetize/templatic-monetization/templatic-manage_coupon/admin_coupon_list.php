...
<h2><?php _e('Manage Coupons',DOMAIN);?>  
<a id="coupon_list" href="<?php echo site_url().'/wp-admin/admin.php?page=monetization&tab=manage_coupon&action=addnew';?>" title="<?php _e('Add a field for Coupon',DOMAIN);?>" name="btnviewlisting" class="add-new-h2" /><?php _e('Add a new coupon',DOMAIN); ?></a>
</h2>
<p class="description"><?php _e('Displaying a list of coupons added on your site where you can manage all the coupons. You will be able to set the time period as well for each coupon. Hence a coupon will not be applicable after it gets expired.',DOMAIN);?></p>

<?php
if(@$_REQUEST['usermetamsg']=='delsuccess')
{
	$message = __('Coupon deleted successfully.',DOMAIN);	
} 
	if(isset($_REQUEST['msgtype']) && $_REQUEST['msgtype']=='add-suc') {
			$message = __('Coupon created successfully.',DOMAIN);
		} elseif(isset($_REQUEST['msgtype']) && $_REQUEST['msgtype'] =='edit-suc') {
			$message = __('coupon updated successfully.',DOMAIN);
		}
