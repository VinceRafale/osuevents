...
class templatic_List_Table extends WP_List_Table 
{
	...
	* Call a function that will return all the data in an array and we will assign that result to a variable $package_data. FIRST OF ALL WE WILL FETCH DATA FROM POST META TABLE STORE THEM IN AN ARRAY $package_data */
	function fetch_package_meta_data( $post_id = '' ,$post_title = '')
	{
		...
		if( $validity_per == 'D' )
		{
			$validity_per = $validity."&nbsp;Days";
		}
		elseif( $validity_per == 'M' )
		{
			$validity_per = $validity."&nbsp;Months";
		}
		else
		{
			$validity_per = $validity."&nbsp;Years";
		}
		if( $status == '1' )
		{
			$package_status = "<font color='green'>Active</font>";
		}
		else
		{
			$package_status = "<font color='red'>Inactive</font>";
		}
		if( $featured == '1' )
		{
			$ftype = "Yes";
		...
		if($package_type ==2){
			$package_type = "Pay per subscription";
		}else{
			$package_type = "Pay per post";
		}
		$meta_data = array(
			'ID'				=> $post_id,
			'title'				=> '<strong><a href="'.site_url().	'/wp-admin/admin.php?page=monetization&action=edit&tab=packages&package_id='.$post_id.'">'.$post_title.'</a></strong>',
	...
	function get_columns()
	{
		$columns = array(
			...
			'package_type' => __('Package type',DOMAIN),
	...
	function process_bulk_action()
	{
		...
		if( 'delete' === $this->current_action() )
		{
			...
			<?php if(count($ids) > 0 )
				  {
					echo count($ids)."&nbsp;"; _e('Packages permanently deleted.',DOMAIN);
				  }
				  else
				  {
