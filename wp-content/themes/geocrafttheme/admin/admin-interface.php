...
function inkthemes_optionsframework_add_admin() {
    ...
    $themename = geocraft_get_option('of_themename');
    $shortname = geocraft_get_option('of_shortname');

    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            $options = geocraft_get_option('of_template');
    ...
    //inkthemes_optionsframework_options_page();
...
function inkthemes_reset_options($options, $page = '') {
    ...
    if ($page == 'optionsframework') {
        geocraft_delete_option('of_options');
    }
}
...
function inkthemes_optionsframework_options_page() {
    ...
    <div class="wrap" id="of_container">
        <div id="of-popup-save" class="of-save-popup">
            <div class="of-save-save"><?php echo OPTION_UPDATED; ?></div>
        </div>
        <div id="of-popup-reset" class="of-save-popup">
            <div class="of-save-reset"><?php echo OPTION_RESET; ?></div>
        </div>
        <form action="" enctype="multipart/form-data" id="ofform">
            <div id="header">
                <div class="logo">
                    <h2><?php echo $themename; ?> <?php echo OPTIONS; ?></h2>
                </div>
            ...
            <div class="save_bar_top">
                <img style="display:none" src="<?php echo ADMINURL; ?>images/loading-bottom.gif" class="ajax-loading-img ajax-loading-img-bottom" alt="Working..." />
                <input type="submit" id="option_submit" value="Save All Changes" class="button-primary" />
        </form>
...
function inkthemes_admin_style(){
 wp_enqueue_style('inkthemes-adminstyle', ADMINURL . 'css/admin-style.css', false);
}
...
function inkthemes_load_only() {
    ...
    wp_register_script('jquery-input-mask', ADMINURL . 'js/jquery.maskedinput-1.2.2.js', array('jquery'));
    wp_enqueue_script('jquery-input-mask');
    wp_enqueue_script('inkthemes-ajaxupload', ADMINURL . 'js/ajaxupload.js', array('jquery'));
    function inkthemes_admin_head() {
        wp_enqueue_style('inkthemes-colorpicker', ADMINURL . 'css/colorpicker.css', false);
        wp_enqueue_script('inkthemes-colorpicker', ADMINURL . 'js/colorpicker.js', array('jquery'));
        ?>
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = geocraft_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = geocraft_get_option($option_id);
                }
                ?>
        ...
        ?>' == 'true'){
            ...
            jQuery('#option_submit').click(function(){
...
function inkthemes_ajax_callback() {
    ...
    if ($save_type == 'upload') {
        ...
        geocraft_update_option($clickedID, $uploaded_file['url']);
        ...
        foreach ($options as $option_array) {
            $id = $option_array['id'];
            $old_value = geocraft_get_option($id);
            $new_value = '';
                ...
                if (is_array($type)) {
                    foreach ($type as $array) {
                        if ($array['type'] == 'text') {
                            ...
                            geocraft_update_option($id, stripslashes($new_value));
                        }
                    }
                } elseif ($new_value == '' && $type == 'checkbox') { // Checkbox Save
                    geocraft_update_option($id, 'false');
                } elseif ($new_value == 'true' && $type == 'checkbox') { // Checkbox Save
                    geocraft_update_option($id, 'true');
                    ...
                    foreach ($option_options as $options_id => $options_value) {
                        ...
                        if (!isset($output[$multicheck_id])) {
                            geocraft_update_option($multicheck_id, 'false');
                        } else {
                            geocraft_update_option($multicheck_id, 'true');
                        }
                    }
                    ...
                    geocraft_update_option($id, $typography_array);
                } elseif ($type == 'border') {
                    ...
                    geocraft_update_option($id, $border_array);
...
function inkthemes_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = geocraft_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'select':
                ...
                $select_value = geocraft_get_option($value['id']);
            ...
            case "color":
                $val = $value['std'];
                $stored = geocraft_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = geocraft_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = geocraft_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                ...
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = geocraft_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
function inkthemes_optionsframework_uploader_function($id, $std, $mod) {
    ...
    $uploader = '';
    $upload = geocraft_get_option($id);

    if ($mod != 'min') {
        $val = $std;
        if (geocraft_get_option($id) != "") {
            $val = geocraft_get_option($id);
        }
